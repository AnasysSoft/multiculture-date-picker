...
export class CalendarComponent implements AfterViewInit, OnDestroy {
  ...
  initializeComponent(opts: IMyOptions, defaultMonth: IMyDefaultMonth, selectedValue: any, inputValue: string, selectorPos: IMySelectorPosition, dc: (dm: IMyDateModel, close: boolean) => void, cvc: (cvc: IMyCalendarViewChanged) => void, rds: (rds: IMyRangeDateSelection) => void, va: (va: ActiveView) => void, cbe: () => void): void {
    console.log('calendar.initializeComponent.options: ', {opts, defaultMonth, selectedValue, inputValue});
    this.opts = opts;
  ...
  initializeView(defaultMonth: IMyDefaultMonth, selectedValue: any, inputValue: string): void {
    ...
    const today: IMyDate = this.utilService.getToday(this.opts.calendarType);
    ...
    if (!dateRange) {
      ...
      validateOpts = {validateDisabledDates: false, selectedValue: this.utilService.getSelectedValue(this.opts.calendarType, selectedValue, dateRange)};
      const date: IMyDate = this.utilService.isDateValid(this.opts.calendarType, inputValue, this.opts, validateOpts);
      ...
      validateOpts = {validateDisabledDates: false, selectedValue: this.utilService.getSelectedValue(this.opts.calendarType, selectedValue, dateRange)};
      const {begin, end} = this.utilService.isDateValidDateRange(this.opts.calendarType, inputValue, this.opts, validateOpts);
  ...
  clearDate(): void {
    const {month, year} = this.utilService.getToday(this.opts.calendarType);
  ...
  generateMonths(): void {
    ...
    for (let i = 1; i <= 12; i += 3) {
      ...
      for (let j = i; j < i + 3; j++) {
        const disabled: boolean = this.utilService.isDisabledMonth(this.opts.calendarType, year, j, this.opts);
        rowData.push({
          nbr: j,
          year,
          name: monthLabels[j],
  ...
  generateYears(inputYear: number): void {
    ...
    for (let i = y; i < y + 25; i += 5) {
      ...
      for (let j = i; j < i + 5; j++) {
        const disabled: boolean = this.utilService.isDisabledYear(this.opts.calendarType, j, this.opts);
        rowData.push({
          year: j,
  ...
  getYearValueByRowAndCol(row: number, col: number): number {
    ...
    if (!years || years.length === 0) {
      const {year} = this.utilService.getToday(this.opts.calendarType);
      return year;
    }
  ...
  setDateViewMonth(isNext: boolean): void {
    ...
    const d: Date = this.utilService.getJsDate(this.opts.calendarType, year, monthNbr, 1);
  ...
  selectDate(date: IMyDate): void {
    ...
    if (dateRange) {
      // Date range
      const isBeginDateInitialized: boolean = this.utilService.isInitializedDate(this.opts.calendarType, this.selectedDateRange.begin);
      const isEndDateInitialized: boolean = this.utilService.isInitializedDate(this.opts.calendarType, this.selectedDateRange.end);
      if (isBeginDateInitialized && isEndDateInitialized) {
        ...
        // second selection
        const firstDateEarlier: boolean = this.utilService.isDateEarlier(this.opts.calendarType, date, this.selectedDateRange.begin);
        if (firstDateEarlier) {
          this.selectedDateRange.begin = date;
          this.rangeDateSelection({
            isBegin: true,
  ...
  getDayNumber(date: IMyDate): number {
    ...
    const d: Date = this.utilService.getJsDate(this.opts.calendarType, year, month, day);
    return d.getDay();
  }
  ...
  generateCalendar(m: number, y: number, notifyChange: boolean): void {
    ...
    const dInThisM: number = this.utilService.datesInMonth(this.opts.calendarType, m, y);
    const dInPrevM: number = this.utilService.datesInPrevMonth(this.opts.calendarType, m, y);
    ...
    for (let i = 1; i < 7; i++) {
      ...
      const weekNbr: number = showWeekNumbers  && firstDayOfWeek === MO ? this.utilService.getWeekNumber(this.opts.calendarType, week[0].dateObj) : 0;
  ...
  setDateViewHeaderBtnDisabledState(m: number, y: number): void {
    ...
    if (disableHeaderButtons) {
      const duDate: IMyDate = {year: m === 1 ? y - 1 : y, month: m === 1 ? 12 : m - 1, day: this.utilService.datesInMonth(this.opts.calendarType, m === 1 ? 12 : m - 1, m === 1 ? y - 1 : y)};
      const dsDate: IMyDate = {year: m === 12 ? y + 1 : y, month: m === 12 ? 1 : m + 1, day: 1};

      dpm = this.utilService.isDisabledByDisableUntil(this.opts.calendarType, duDate, disableUntil)
        && !this.utilService.isPastDatesEnabled(this.opts.calendarType, duDate, enableDates);
      dnm = this.utilService.isDisabledByDisableSince(this.opts.calendarType, dsDate, disableSince)
        && !this.utilService.isFutureDatesEnabled(this.opts.calendarType, dsDate, enableDates);
    }
  ...
  setYearViewHeaderBtnDisabledState(yp: number, yn: number): void {
    ...
    if (disableHeaderButtons) {
      ...
      dpy = this.utilService.isDisabledByDisableUntil(this.opts.calendarType, duDate, disableUntil)
        && !this.utilService.isPastDatesEnabled(this.opts.calendarType, duDate, enableDates);
      dny = this.utilService.isDisabledByDisableSince(this.opts.calendarType, dsDate, disableSince)
        && !this.utilService.isFutureDatesEnabled(this.opts.calendarType, dsDate, enableDates);
    }
