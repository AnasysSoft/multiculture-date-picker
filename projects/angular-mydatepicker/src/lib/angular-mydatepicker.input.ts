...
export class AngularMyDatePickerDirective implements OnChanges, OnDestroy, ControlValueAccessor, Validator {
  ...
  constructor(private localeService: LocaleService,
    ...
    console.log('constructor.calendar: ', this.calendar);
    this.opts = this.config.getDefaultConfig(this.calendar);
    this.parseOptions(this.opts);
  }

  @HostListener(KEYUP, ["$event"]) onKeyUp(event: any) {
    const keyCode: number = this.utilService.getKeyCodeFromEvent(this.opts.calendarType, event);
    ...
    if (keyCode === KeyCode.esc) {
      ...
      let valid: boolean;
      let validateOpts: IMyValidateOptions;
      if (!dateRange) {
        validateOpts = {validateDisabledDates: true, selectedValue: this.utilService.getSelectedValue(this.opts.calendarType, this.selectedValue, false)};
        const date: IMyDate = this.utilService.isDateValid(this.opts.calendarType, value, this.opts, validateOpts);
        valid = this.utilService.isInitializedDate(this.opts.calendarType, date);
        if (valid) {
          dateModel = this.utilService.getDateModel(this.opts.calendarType, date, null, dateFormat, monthLabels, dateRangeDatesDelimiter);
        }
      }
      else {
        validateOpts = {validateDisabledDates: true, selectedValue: this.utilService.getSelectedValue(this.opts.calendarType, this.selectedValue, true)};
        const range = this.utilService.isDateValidDateRange(this.opts.calendarType, value, this.opts, validateOpts);
        const {begin, end} = range;
        valid = this.utilService.isInitializedDate(this.opts.calendarType, begin) && this.utilService.isInitializedDate(this.opts.calendarType, end);
        if (valid) {
          dateModel = this.utilService.getDateModel(this.opts.calendarType, null, range, dateFormat, monthLabels, dateRangeDatesDelimiter);
        }
      }
    ...
    if (inputFieldValidation) {
      ...
      if (!dateRange) {
        ...
        if (valid && this.hostText !== value) {
          // Valid date
          const dateModel: IMyDateModel = this.utilService.getDateModel(this.opts.calendarType, date, null, dateFormat, monthLabels, dateRangeDatesDelimiter);
        ...
        validateOpts = {validateDisabledDates: true, selectedValue: this.utilService.getSelectedValue(this.opts.calendarType, this.selectedValue, true)};
        const dateRange: IMyDateRange = this.utilService.isDateValidDateRange(this.opts.calendarType, value, this.opts, validateOpts);
        const {begin, end} = dateRange;
        valid = this.utilService.isInitializedDate(this.opts.calendarType, begin) && this.utilService.isInitializedDate(this.opts.calendarType, end);
        if (valid && this.hostText !== value) {
          // Valid date range
          const dateModel: IMyDateModel = this.utilService.getDateModel(this.opts.calendarType, null, dateRange, dateFormat, monthLabels, dateRangeDatesDelimiter);
          this.emitDateChanged(dateModel);
          this.updateModel(dateModel);
          if (closeSelectorOnDateSelect) {
            this.closeSelector(CalToggle.CloseByDateSel);
  ...
  public setLocaleOptions(): void {
    const opts: IMyOptions = this.localeService.getLocaleOptions(CalendarType.Gregorian, this.locale);
  ...
  public writeValue(value: any): void {
    ...
    if (!value) {
      ...
      if (!date) {
        date = this.utilService.jsDateToMyDate(this.opts.calendarType, jsDate);
      }

      const formatted: string = this.utilService.formatDate(this.opts.calendarType, date, dateFormat, monthLabels);

      validateOpts = {validateDisabledDates: false, selectedValue: this.utilService.getSelectedValue(this.opts.calendarType, this.selectedValue, false)};
      const valid: boolean = this.utilService.isInitializedDate(this.opts.calendarType, this.utilService.isDateValid(this.opts.calendarType, formatted, this.opts, validateOpts));
      if (valid) {
        this.setHostValue(formatted);
        this.emitInputFieldChanged(formatted, valid);
        this.setSelectedValue(this.utilService.getDateModel(this.opts.calendarType, date, null, dateFormat, monthLabels, dateRangeDatesDelimiter));
      ...
      if (!beginDate || !endDate) {
        beginDate = this.utilService.jsDateToMyDate(this.opts.calendarType, beginJsDate);
        endDate = this.utilService.jsDateToMyDate(this.opts.calendarType, endJsDate);
      }

      const formatted: string = this.utilService.formatDate(this.opts.calendarType, beginDate, dateFormat, monthLabels) + dateRangeDatesDelimiter +
        this.utilService.formatDate(this.opts.calendarType, endDate, dateFormat, monthLabels);
      validateOpts = {validateDisabledDates: false, selectedValue: this.utilService.getSelectedValue(this.opts.calendarType, this.selectedValue, true)};
      const {begin, end} = this.utilService.isDateValidDateRange(this.opts.calendarType, formatted, this.opts, validateOpts);
      const valid: boolean = this.utilService.isInitializedDate(this.opts.calendarType, begin) && this.utilService.isInitializedDate(this.opts.calendarType, end);
      if (valid) {
        this.setHostValue(formatted);
        this.emitInputFieldChanged(formatted, valid);

        const dateRange: IMyDateRange = {begin: beginDate, end: endDate};
        this.setSelectedValue(this.utilService.getDateModel(this.opts.calendarType, null, dateRange, dateFormat, monthLabels, dateRangeDatesDelimiter));
  ...
  public validate(c: AbstractControl): { [p: string]: any } {
    ...
    if (!this.opts.dateRange) {
      ...
      validateOpts = {validateDisabledDates: true, selectedValue: this.utilService.getSelectedValue(this.opts.calendarType, this.selectedValue, true)};
      const {begin, end} = this.utilService.isDateValidDateRange(this.opts.calendarType, value, this.opts, validateOpts);
  ...
  public openCalendar(): void {
    console.log('openCalendar: ', { calendar: this.calendar, opts: this.opts, options: this.options});
    if (this.disabled) {
      return;
    }
    ...
    if (this.cRef === null) {
      ...
      this.cRef.instance.initializeComponent(
        ...
        (dm: IMyDateModel, close: boolean) => {
          ...
          this.emitInputFieldChanged(this.utilService.getFormattedDate(this.opts.calendarType, dm), true);
          this.updateModel(dm);
          if (close) {
            this.closeSelector(CalToggle.CloseByDateSel);
          }
  ...
  public clearDate(): void {
    ...
    this.emitDateChanged({
      ...
      singleDate: {
        date: this.utilService.resetDate(this.opts.calendarType),
        jsDate: null,
        formatted: EMPTY_STR,
        epoc: 0
      },
      dateRange: {
        beginDate: this.utilService.resetDate(this.opts.calendarType),
        beginJsDate: null,
        beginEpoc: 0,
        endDate: this.utilService.resetDate(this.opts.calendarType),
        endJsDate: null,
        endEpoc: 0,
        formatted: EMPTY_STR
      }
  ...
  private updateModel(model: IMyDateModel): void {
    this.setHostValue(this.utilService.getFormattedDate(this.opts.calendarType, model));
  ...
  public changeOptions(options: IMyOptions): void {
