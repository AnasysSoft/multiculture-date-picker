...
export class JalaliCalendar implements ICalendarService {
  ...
  isDateValid(dateStr: string, options: IMyOptions, validateOpts: IMyValidateOptions): IMyDate {
    console.log('JalaliCalendar.isDateValid: ', {dateStr, options, validateOpts});
    const {dateFormat, minYear, maxYear, monthLabels} = options;

    const returnDate: IMyDate = this.resetDate();
    const datesInMonth: Array<number> = [31, 31, 31, 31, 31, 31, 30, 30, 30, 30, 30, 29];
  ...
  getDateValue(dateStr: string, dateFormat: string, delimeters: Array<string>): Array<IMyDateFormat> {
    console.log('JalaliCalendar.getDateValue: ', {dateFormat, delimeters});
  ...
  getMonthNumberByMonthName(df: IMyDateFormat, monthLabels: IMyMonthLabels): number {
    console.log('JalaliCalendar.getMonthNumberByMonthName: ', {df, monthLabels});
    if (df.value) {
  ...
  getNumberByValue(df: IMyDateFormat): number {
    console.log('JalaliCalendar.getNumberByValue: ', {df});
    if (!/^\d+$/.test(df.value)) {
      return -1;
    }
  ...
  parseDefaultMonth(monthString: string): IMyMonth {
    console.log('JalaliCalendar.parseDefaultMonth: ', {monthString});
    console.log('jalali.parseDefaultMonth: ', monthString);
  ...
  getDateModel(date: IMyDate, dateRange: IMyDateRange, dateFormat: string, monthLabels: IMyMonthLabels, rangeDelimiter: string, dateStr: string = EMPTY_STR): IMyDateModel {
    console.log('JalaliCalendar.getDateModel: ', {date, dateRange, dateFormat, monthLabels, rangeDelimiter, dateStr});
  ...
  formatDate(date: IMyDate, dateFormat: string, monthLabels: IMyMonthLabels): string {
    console.log('JalaliCalendar.formatDate: ', {date, dateFormat, monthLabels});
  ...
  getToday(): IMyDate {
    console.log('JalaliCalendar.getToday: ');
    return this.gregorianToJalaliDate(new Date());
  }

  getDayNumber(date: IMyDate): number {
    return this.myDateToJsDate(date).getDay();
  }
  ...
  jsDateToMyDate(date: Date): IMyDate {
    console.log('JalaliCalendar.jsDateToMyDate: ', {date});
    return this.gregorianToJalaliDate(date);
  }

  myDateToJsDate(date: IMyDate): Date {
    console.log('JalaliCalendar.myDateToJsDate: ', {date});
  ...
  datesInMonth(m: number, y: number): number {
    if (m <= 6) return 31;
    if (m <= 11) return 30;
    return moment.jIsLeapYear(y) ? 30 : 29;
  }
  ...
  getJsDate(year: number, month: number, day: number): Date {
    console.log('JalaliCalendar.getJsDate: ', {year, month, day});
  ...
  private jalaliToGregorianDate(year: number, month: number, day: number) : Date {
    console.log('JalaliCalendar.jalaliToGregorianDate: ', {year, month, day});
    const date = moment.from(`${year}/${month}/${day}`, "jYYYY/jM/jD");

    const result = new Date(date.year(), date.month() - 1, date.date(), 0, 0, 0, 0);
    console.log('JalaliCalendar.jalaliToGregorianDate.result: ', result);
    return result;
  }

  private jalaliToGregorianMyDate(year: number, month: number, day: number) : IMyDate {
    console.log('JalaliCalendar.jalaliToGregorianMyDate: ', {year, month, day});
    const date = moment.from(`${year}/${month}/${day}`, "jYYYY/jM/jD");
    const result = {year: date.year(), month: date.month(), day: date.date()};
    console.log('JalaliCalendar.jalaliToGregorianDate.result: ', result);
    return result;
  }

  private gregorianToJalaliDate(date: Date) : IMyDate {
    console.log('JalaliCalendar.gregorianToJalaliDate: ', {date});
    const m = moment(date);
    const result = {year: m.jYear(), month: m.jMonth() + 1, day: m.jDate()};
    console.log('JalaliCalendar.jalaliToGregorianDate.result: ', {moment: m.format("jYYYY/jMM/jDD"), result});
    return result;
  }

  private gregorianToJalaliMyDate(date: IMyDate) : IMyDate {
    console.log('JalaliCalendar.gregorianToJalaliMyDate: ', {date});
    const result = this.gregorianToJalaliDate(this.myDateToJsDate(date));
    console.log('JalaliCalendar.jalaliToGregorianDate.result: ', result);
    return result;
  }
}
